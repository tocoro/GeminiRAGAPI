...
const DocumentList: React.FC<DocumentListProps> = ({ selectedStore, documents, isLoading, processingFile, onUpload, onDelete }) => {
    ...
    const handleSaveBlog = (title: string, content: string) => {
        // Create a text file from the blog content
        // Appending .txt ensures Gemini treats it as a text document
        const safeTitle = title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
        const fileName = `${safeTitle}.txt`;
        const blob = new Blob([content], { type: 'text/plain' });
        const file = new File([blob], fileName, { type: 'text/plain' });

        // Add metadata to indicate it's a knowledge update
        const blogMetadata: CustomMetadata[] = [
            { key: 'type', stringValue: 'knowledge_update' },
            { key: 'original_title', stringValue: title }
        ];

        onUpload(file, blogMetadata);
    };
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold truncate pr-2" title={selectedStore.displayName}>Documents</h2>
                <div className="flex space-x-2 flex-shrink-0">
                    <button
                        onClick={handleBlogClick}
                        className="flex items-center px-3 py-2 bg-gem-teal hover:bg-teal-600 rounded-md text-white transition-colors disabled:bg-gem-mist disabled:cursor-not-allowed text-sm font-medium"
                        disabled={!!processingFile}
                        aria-label="Write Update"
                        title="Write a text update/blog for this store"
                    >
                        <PencilIcon />
                        <span className="ml-1 hidden sm:inline">Write Update</span>
                    </button>
                    <button
                        onClick={handleUploadClick}
                        className="flex items-center px-3 py-2 bg-gem-blue hover:bg-blue-500 rounded-md text-white transition-colors disabled:bg-gem-mist disabled:cursor-not-allowed text-sm font-medium"
                        disabled={!!processingFile}
                        aria-label="Upload document"
                        title="Upload a new document to this store"
                    >
                        <UploadIcon />
                        <span className="ml-1 hidden sm:inline">Upload File</span>
                    </button>
                </div>
            </div>
            ...
            {isUploadModalOpen && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60]" role="dialog" aria-modal="true" aria-labelledby="upload-doc-title">
            ...
            {isLoading && !documents.length ? (
                ...
                <div className="flex-grow flex items-center justify-center text-center text-gem-offwhite/60">
                    ...
                    <p className="mt-2">Upload a file or write an update<br/>to start your knowledge base.</p>
                </div>
            ) : (
                <ul className="space-y-2 overflow-y-auto">
                    {documents.map((doc) => {
                        // Check if it's a text file (likely a blog/update)
                        const isTextFile = doc.name.endsWith('.txt') || doc.displayName.endsWith('.txt');
                        return (
                            <li key={doc.name} className={`p-3 rounded-md group ${isTextFile ? 'bg-gem-mist/70 border-l-4 border-gem-teal' : 'bg-gem-mist'}`}>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center min-w-0">
                                        {isTextFile ? (
                                            <span className="mr-2 text-gem-teal" title="Knowledge Update / Text">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clipRule="evenodd" />
                                                </svg>
                                            </span>
                                        ) : (
                                            <span className="mr-2 text-gem-blue" title="Document">
                                                 <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
